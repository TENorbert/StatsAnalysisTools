...
*/
// Gaussian Time for QCD Bkg
Double_t background(Double_t *x, Double_t *par){
Double_t arg = 0;
if(par[2]!=0) arg = (x[0] - par[1])/par[2];
Double_t fitval = par[0]*TMath::Exp(-0.5*arg*arg);
return fitval; 
//return TMath::Exp(-t*t/(p0*p0))/sqrt(2*TMath::Pi()*p0);
}

//Exponentially falling time Signal
Double_t signal(Double_t *x, Double_t *par){

return  TMath::Exp(-x[0]/par[3])/par[3];
}
...
// Sum of background and peak function
Double_t fitFunction(Double_t *x, Double_t *par) {
  return background(x,par) + signal(x,&par[3]);
}
...
void FittingDemo() {
 //Bevington Exercise by Peter Malzacher, modified by Rene Brun
   ...
   const int nBins = 32;
   Double_t data[nBins] = {-6,-5,-4,-3,-2,-1,0,1,2,3,
...
*/
   ...
   TH1F *histo = new TH1F("histo",
      "Exponential Signal on Gaussian Background",32,-6,25);
   ...
   TF1 *fitFcn = new TF1("fitFcn",fitFunction,-6,25,6);
   fitFcn->SetNpx(500);
   ...
   fitFcn->SetParameter(0,0.2); // width
   fitFcn->SetParameter(0,1); // peak
 
   histo->Fit("fitFcn","V+","ep");
   
   // improve the picture:
   //TF1 *backFcn = new TF1("backFcn",background,0,3,3);
   TF1 *backFcn = new TF1("backFcn",background,-6,25,3);
   backFcn->SetLineColor(kRed);
   //TF1 *signalFcn = new TF1("signalFcn",lorentzianPeak,0,3,3);
   TF1 *signalFcn = new TF1("signalFcn",signal,-6,25,3);
